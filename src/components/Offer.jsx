            ...
            return (
              <>
                <Table.Row className="bg-white dark:border-gray-700 dark:bg-gray-800">
                  ...
                  <Table.Cell>
                    <Link to={item.id} onClick={() => handleShow(item)}>
                  ...
                  <Table.Cell onClick={() => updateStatusCancel()}>
                    {item.status == "Waiting Accept" ? (
                      <div className="w-full flex items-center gap-2">
                        <button
                          onClick={() => updateStatusCancel(item.id)}
                          className="px-4 py-1.5 rounded text-white bg-red-500 text-xs"
                        >
                          Cancel
                        </button>
                        <button
                          onClick={() => updateStatusApprove(item.id)}
                    ...
                    {item.status == "Success" ? (
                      <p className="text-semibold text-green-500">Finish</p>
                    ) : (
                      <></>
                    )}
                    {item.status == "Cancel" ? (
                      <p className="text-semibold text-red-500">Cancel</p>
                    ) : (
                      <></>
                    )}
                    {item.status == "Finish" ? (
